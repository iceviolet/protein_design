 ...
 * @param vergil
 */
void DesignP4(Vergil* vergil, std::vector<double> parameters);
...
int main(int argc, char **argv) {
	...
	if (argc != 9) {
		Log->error(FLERR, "The number of arguments passed to the program must be 8");
	}
...
void DesignP4212(Vergil* vergil) {
	...
	std::string pdb_filename = input_directory
			+ "/C4459a_chainA_fixed_adg_NH2cap_for_vergil.pdb";
	...
	for (double a = 37.3; a < 43.5; a += 0.1) {
		...
		std::string typelist[] = { "ASP", "GLN", "GLU", "ILE", "ARG", "GLN",
				"MET", "ALA", "GLU", "TRP", "ILE", "LYS", "LYS", "MET", "ALA",
				"GLN", "MET", "ILE", "GLU", "LYS", "MET", "ALA", "HIS", "ARG",
				"ILE", "ASP", "ARG", "GLU", "ALA", "NH2" };

		for (Domain::SectionIterator jt =
				vergil->domain()->SectionIterator_Begin();
				jt != vergil->domain()->SectionIterator_End(); ++jt) {
			size_t i = 0;
			for (Section::SiteIterator it = jt->SiteIterator_Begin();
		...
		vergil->domain()->TransformBy(
				Geometry::Transformation(Vector3(0, 0, 1), 45.0, 0.0));

		//Set up symmetry and build lattice
		vergil->domain()->set_unit_cell_parameters("P4212", a, a, 1000, 90.0,
				90.0, 90.0);
		...
		FunctionMeanFieldEnergyLattice energy(vergil->domain(),
				symmetry_related_elements);
		energy.set_pairwise_energy_cap_values(twobody_energy_cap,
				twobody_energy_cap);
		...
		FunctionFreeEnergyWithReference free_energy(&energy, design_beta,
				&unfolded_compute);
		...
		std::string output_file = output_directory + "/C4459_P4212_nofixcore_a"
				+ Log->to_str(a) + "_mp_type_wCap";
		//vergil->StandardOutput(output_file);

		for (Domain::SiteIterator it = vergil->domain()->SiteIterator_Begin();
				it != vergil->domain()->SiteIterator_End(); ++it) {
...
void DesignP4(Vergil* vergil, std::vector<double> parameters) {
	...
	double theta_min = parameters[2];
	double theta_max = parameters[3];
	double d_min = parameters[4];
	double d_max = parameters[5];
	double alpha_min = parameters[6];
	double alpha_max = parameters[7];
	...
	FILE* outfile = fopen(outfile_energy.c_str(), "w");
	fprintf(outfile,
			"UnitCell_length_a(nm), theta(deg), d(A), alpha(deg), Internal_energy(Kcal/mol)\n");
	fclose(outfile);
	...
	for (double a = a_min; a < a_max + 0.1; a += 0.1) {
		for (double theta = theta_min; theta < theta_max + 1; ++theta) {
			for (double d = d_min; d < d_max + 0.1; d += 0.1) {
				for (double alpha = alpha_min; alpha < alpha_max + 1; ++alpha) {

					std::string scaffold = "protein and backbone or site 30";
					...
					//Type each site in the domain using the original P422_Cu_1 sequence
					//DQEIRQM AEWIKKM AQMIDKM AHRIDRE A-NH2
					std::string typelist[] = { "ASP", "GLN", "GLU", "ILE",
							"ARG", "GLN", "MET", "ALA", "GLU", "TRP", "ILE",
							"LYS", "LYS", "MET", "ALA", "GLN", "MET", "ILE",
							"GLU", "LYS", "MET", "ALA", "HIS", "ARG", "ILE",
							"ASP", "ARG", "GLU", "ALA", "NH2" };

					for (Domain::SectionIterator jt =
							vergil->domain()->SectionIterator_Begin();
							jt != vergil->domain()->SectionIterator_End();
							++jt) {
						size_t i = 0;
					...
					const Matrix m_z = Geometry::Transformation(
							Vector3(0, 0, 1), theta * DEGREES_TO_RADIANS);
					...
					const Vector3 offset = Vector3(
							d * cos(alpha * DEGREES_TO_RADIANS),
							d * sin(alpha * DEGREES_TO_RADIANS), 0);
					const Matrix m_move = Geometry::Transformation(
							Vector3(0, 0, 0), 0, offset);
					vergil->domain()->TransformBy(m_move);

					//Set up symmetry and build lattice
					vergil->domain()->set_unit_cell_parameters("P4", a, a, 1000,
							90.0, 90.0, 90.0);
					SymmetryGenerator<Domain> symexp("P4", a, a, 1000, 90.0,
							90.0, 90.0);
					symexp.BuildLattice(*vergil->domain(), -1, -1, 0, 1, 1, 0);
					...
					std::string output_file = output_directory
							+ "/C4459_P4_bundle_nofixcore_a" + Log->to_str(a) + "_theta" + Log->to_str(theta) +
							"_d" + Log->to_str(d) + "_alpha" + Log->to_str(alpha) + "_mp_type_wCap";
					//vergil->StandardOutput(output_file);

					if (meanfield_energy < -700) {
